...
import { listUsersVip, deleteUser } from '../actions/userActions.js'
import Countdown, { zeroPad } from 'react-countdown'
import moment from 'moment'
import { SearchBoxVip } from '../components/SearchBox.js'
// import  SearchBoxVip  from '../components/SearchBar.js'


const UserListVipScreen = ({ history, match }) => {
  const keyword = match.params.keyword
  const pageNumber = match.params.pageNumber || 1

  const dispatch = useDispatch()

  const userListVip = useSelector((state) => state.userListVip)

  const { loading, error, count, users, page, pages } = userListVip
  ...
  useEffect(() => {
    dispatch(listUsersVip(keyword, pageNumber))
  ...
  const renderer = ({ days, hours, minutes, seconds, completed }) => {
    if (days === 5 && hours === 23 && minutes === 10 && seconds === 0) {
      dispatch(listUsersVip(keyword, pageNumber))
    }
...
export default UserListVipScreen
