...
import {
  predictionVipDetailsAction,
  predictionVipfixtureAction,
  updatePredictionVipAction,
  fixtureVipAction,
  deleteSingleFixtureVipAction,
  deleteFixtureVipAction,
...
const PredictionVipEditScreen = ({ match, history }) => {
  ...
  const predictionVipDetails = useSelector(
    (state) => state.predictionVipDetails
  )
  const { loading, error, prediction } = predictionVipDetails
  ...
  const predictionVipUpdate = useSelector((state) => state.predictionVipUpdate)
  const {
  ...
  console.log(JSON.stringify(user) + 'Prediction Vip Edit User')
  console.log(JSON.stringify(user) + 'Prediction Vip Edit User')

  useEffect(() => {
    if (successUpdate) {
      dispatch({ type: PREDICTION_VIP_UPDATE_RESET })
      history.push('/vip/admin/predictionviplist')
    } else {
      if (!prediction) {
        console.log(prediction)
        dispatch(predictionVipDetailsAction(predictionId))
      } else {
  ...
  const submitHandler = (e) => {
    e.preventDefault()
    dispatch(
      updatePredictionVipAction({ _id: predictionId, title, isPublished })
    )
  }
  ...
  const handleShow2 = () => {
    dispatch(predictionVipfixtureAction(predictionId))
    setShow2(true)
  }
  ...
  const submitFixtureHandler = (e) => {
    e.preventDefault()
    dispatch(fixtureVipAction({ league, home, away, tipsOdds }))
    setShow(false)
    setHome('')
    setAway('')
    setTipsOdds('')
  ...
  const deleteHandler2 = (id) => {
    if (window.confirm('Are you sure')) {
      dispatch(deleteFixtureVipAction(id))
      dispatch(predictionVipfixtureAction(predictionId))
    }
  }
  ...
  const resetHandler = (e) => {
    ...
    localStorage.removeItem('predictionvip_Details')
    history.push('/vip/admin/predictionviplist')
  }
    ...
    <>
      <Modal
        ...
        <Modal.Footer>
          ...
          {isPublished ? <Button variant="secondary" onClick={handleClose}>
          Add Fixture
          </Button> : (
      ...
      <Modal
        ...
        <Modal.Footer>
          ...
          {isPublished ? <Button variant="secondary" onClick={handleClose2}>
          Delete All
          </Button> : (
            <Button
              variant="danger"
...
export default PredictionVipEditScreen
