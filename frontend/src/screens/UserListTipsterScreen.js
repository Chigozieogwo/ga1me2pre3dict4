...
import { listUsersTipster, deleteUser } from '../actions/userActions.js'
import Countdown, { zeroPad } from 'react-countdown'
import moment from 'moment'

const UserListTipsterScreen = ({ history, match }) => {
  const pageNumber = match.params.pageNumber || 1

  const dispatch = useDispatch()

  const userListTipster = useSelector((state) => state.userListTipster)

  const { loading, error, users, page, pages } = userListTipster
  ...
  useEffect(() => {
    dispatch(listUsersTipster(pageNumber))
  }, [dispatch, pageNumber])
...
export default UserListTipsterScreen
